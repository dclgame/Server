...
class MsgHead : public MsgHeadBase
{
public:
	MsgHead()
	{
	...
	virtual int DeCode(const char* strData)
	{
		...
		munSize = NTOHL(nPackSize) - HEAD_LENGTH;
		nOffset += sizeof(munSize);
...
typedef std::function<void(const SOCKID nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)> NET_RECEIVE_FUNCTOR;
typedef std::shared_ptr<NET_RECEIVE_FUNCTOR> NET_RECEIVE_FUNCTOR_PTR;

typedef std::function<void(const SOCKID nSockIndex, const NET_EVENT nEvent, Net* pNet)> NET_EVENT_FUNCTOR;
...
{
public:
	NetObject(Net* pNet, SOCKID fd, sockaddr_in& addr, struct bufferevent* pBev)
	{
		mnLogicState = 0;
		mnGameID = 0;
		nFD = fd;
...
public:
	...
	virtual bool SendMsgWithOutHead(const int16_t nMsgID, const char* msg, const uint32_t nLen, const SOCKID nSockIndex = 0) = 0;
	...
	virtual bool CloseNetObject(const SOCKID nSockIndex) = 0;
	virtual NetObject* GetNetObject(const SOCKID nSockIndex) = 0;
	virtual bool AddNetObject(const SOCKID nSockIndex, NetObject* pObject) = 0;
